...
//		scanf("%d", &a[i]);
//	}
//	for (j = 0; j < 9; j++)//9趟冒泡排序
//	{
//		for (i = 0; i < 9-j; i++)//每一趟的过程
//		{
//			if (a[i] > a[i+1])
//			{
//				k = a[i];
...
//	again:
//	printf("请注意，你的电脑将在60秒后关机，若输入：好好学习，就取消关机\n");
//    scanf("%s", input);
//	if (strcmp(input, "好好学习") == 0)
//	{
//		system("shutdown -a");
//	}
//	else {
...
{
	int i = 0;
	for (i = 0; i < sz - 1; i++)//sz个数，确定了sz-1趟排序
	{
		int judge = 1;//假设已经排好了
		int j = 0;
		for (j = 0; j < sz-1-i; j++)//每一趟冒泡排序的过程
		{
			if (arr[j] > arr[j + 1])
			{
				int swap = arr[j];
